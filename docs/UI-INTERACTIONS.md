...
- [ ] Empty bullets are transient: an item with no content (e.g. a freshly-indented bullet under another) stays live and editable, but is dropped on save — it is not written to disk and does not survive a reload. Typing into it persists it normally. An empty `- ` under text has no faithful plain-markdown form (CommonMark reads it as a setext heading underline), so dropping it avoids both a save-integrity warning and the parent line being corrupted into a heading on reload
...
- [ ] Editing a note with content that round-trips cleanly shows only the normal `Saving…` status — no warning. The comparison ignores benign normalizations markdown applies by design — blank-line gaps, paragraph-break vs. hard-break (interchangeable under markdown-it `breaks: true`), leading/trailing whitespace trimmed at paragraph/heading edges (e.g. a trailing space at end of line, a leading space in a bullet; `codeBlock` whitespace is preserved), adjacent same-type lists (which markdown reflows into one — e.g. a blank line splitting a list mid-edit), and empty list items / lists left empty by their removal (the serializer drops transient empty bullets) — so ordinary edits never false-positive; calibrated against the full notes corpus (0 false positives)
